...
public:
  ...
  double elapsed_time() const { return m_stop - m_start; }
...
NTL::ZZ noPackingMultiplyAndSum(long u[], long v[], FHESecKey sk,
                                FHEPubKey pk) {
  ...
  // Decrypt the first position of the vector, which holds the value of the
  // scalar product
  ZZX result;
...
NTL::ZZ invertAndMultiply(long u[], long v[], FHESecKey sk, FHEPubKey pk) {
  ...
  // Note that the elements from v are "inverted" when encoded into the
  // coefficients of V(x)
  for (int i = 0; i < VEC_SIZE; i++) {
...
NTL::ZZ multiplyAndTotalSum(long u[], long v[], FHEPubKey pk, FHESecKey sk,
                            FHEcontext &context) {
...
int main(int argc, char **argv) {
  ...
  tMethod1.stop();
  std::cout << "Muliply and sum arrays without packing method result: "
            << method1Result << ". Done in " << tMethod1.elapsed_time() << "s."
            << std::endl;
  ...
  tMethod2.stop();
  std::cout << "Invert and multiply method result: " << method2Result
            << ". Done in " << tMethod2.elapsed_time() << "s." << std::endl;
  ...
  tMethod3.stop();
  std::cout << "Multiply and totalSum method result: " << method3Result
            << ". Done in " << tMethod3.elapsed_time() << "s." << std::endl;
}

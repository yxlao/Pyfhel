...
#   You can view how to run this program (executed in the directory Pyfhel/src/Demo_Pyfhel),
#   with the command: python Demo_Pyfhel -h
#   --------------------------------------------------------------------
...
# v_powern2_decrypt_result is a list of list ie [[a, b, c,...]], so we

# want to flatten it to obtain [a, b, c,...].
v_powern2_decrypt_result_flatten = list(
    itertools.chain.from_iterable(v_powern2_decrypt_result))
print("Decrypt(Encrypt(v1) ** 4) -> ", v_powern2_decrypt_result_flatten)
# Perform the Power4 on the unencrypted vectors.
v_Powern2 = [
    a * b * c * d
    for a, b, c, d in izip(v_powern2, v_powern2, v_powern2, v_powern2)
]

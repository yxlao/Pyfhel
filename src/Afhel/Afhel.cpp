...
// ------------------------------ CRYPTOGRAPHY --------------------------------
// KEY GENERATION
void Afhel::keyGen(long p, long r, long c, long d, long sec, long w, long L,
                   long m, long R, long s, const vector<long> &gens,
                   const vector<long> &ords) {
  if (flagPrint) {
    std::cout << "Afhel::keyGen START" << endl;
  }
  ...
  if (flagPrint) {
    std::cout << "Afhel::keyGen COMPLETED" << endl;
  }
}
...
string Afhel::encrypt(vector<long> plaintext) {
  ...
  // TODO: create a vector of size nddSlots and fill it first with values from
  // plaintext, then with zeros
  ea->encrypt(cyphertext, *publicKey, plaintext); // Encrypt plaintext
  string id1 = store(&cyphertext);
  if (flagPrint) {
    std::cout << "  Afhel::encrypt({ID" << id1 << "}[" << plaintext << "])"
              << endl;
  }
  return id1;
}
...
// CUMULATIVE SUM
void Afhel::cumSum(string id1) { totalSums(*ea, ctxtMap.at(id1)); }

// SQUARE
void Afhel::square(string id1) { ctxtMap.at(id1).square(); }

// CUBE
void Afhel::cube(string id1) { ctxtMap.at(id1).cube(); }

// NEGATE
void Afhel::negate(string id1) { ctxtMap.at(id1).negate(); }
...
// ROTATE
void Afhel::rotate(string id1, long c) { ea->rotate(ctxtMap.at(id1), c); }

// SHIFT
void Afhel::shift(string id1, long c) { ea->shift(ctxtMap.at(id1), c); }
...
bool Afhel::restoreEnv(string fileName) {
  ...
  try {
    ...
    publicKey =
        (FHEPubKey *)secretKey; // Reconstruct Public Key from Secret Key
...
long Afhel::numSlots() { return ea->size(); }
...
Ctxt Afhel::retrieve(string id1) { return ctxtMap.at(id1); }

...
void noPackingSum(long u[], long v[], long result[], FHESecKey sk,
                  FHEPubKey pk) {
  ...
  std::cout << "HElib No packing encryption: "
            << tNoPackingEncryption.elapsed_time() << "s." << std::endl;
  ...
  tNoPackingSum.stop();
  std::cout << "HElib No packing sum: " << tNoPackingSum.elapsed_time() << "s."
            << std::endl;
  ...
  std::cout << "HElib No packing decryption: "
            << tNoPackingDecryption.elapsed_time() << "s." << std::endl;
}

void packingPolySum(long u[], long v[], long result[], FHESecKey sk,
                    FHEPubKey pk) {
  ...
  std::cout << "HElib poly packing encryption: "
            << tPackingPolyEncryption.elapsed_time() << "s." << std::endl;
  ...
  std::cout << "HElib poly packing sum: " << tPackingPolySum.elapsed_time()
            << "s." << std::endl;
  ...
  std::cout << "HElib poly packing decryption: "
            << tPackingPolyDecryption.elapsed_time() << "s." << std::endl;
}

void packingSubfieldSum(long u[], long v[], long result[], FHEPubKey pk,
                        FHESecKey sk, FHEcontext &context) {
  ...
  std::cout << "HElib subfield packing encryption: "
            << tPackingSubfieldEncryption.elapsed_time() << "s." << std::endl;
  ...
  std::cout << "HElib subfield packing sum: "
            << tPackingSubfieldSum.elapsed_time() << "s." << std::endl;
  ...
  std::cout << "HElib subfield packing decryption: "
            << tPackingSubfieldDecryption.elapsed_time() << "s." << std::endl;
}
...
int main(int argc, char **argv) {
  ...
  m = FindM(k, L, c, p, d, s,
            0); // Find a value for m given the specified values
  std::cout << "m = " << m << std::endl;

  std::cout << "Initializing context... " << std::flush;
  FHEcontext context(m, p, r); // Initialize context
  buildModChain(context, L,
                c); // Modify the context, adding primes to the modulus chain
  ...
  // Arrays whose elements will be the coefficients of the polynomials U(x) and
  // V(x)
  long u[VEC_SIZE];
  long v[VEC_SIZE];
  ...
  tInit.stop();
  std::cout << "Time taken for the initialization: " << tInit.elapsed_time()
            << std::endl;
  ...
  tMethod1.stop();
  std::cout << "HElib without packing method done in "
            << tMethod1.elapsed_time() << "s." << std::endl;
  ...
  tMethod2.stop();
  std::cout << "HElib with polynomial packing method done in "
            << tMethod2.elapsed_time() << "s." << std::endl;
  ...
  tMethod3.stop();
  std::cout << "HElib with subfield packing method done in "
            << tMethod3.elapsed_time() << "s." << std::endl;
}
